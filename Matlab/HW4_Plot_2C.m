...
type = {'Newcomen', 'Smeaton', 'Smeaton', 'Watt', 'Watt', 'Woolf compound'};
...
b = [sum_y; sum_xy; sum_x2y]; % right side of the equation
...
% Plot
figure
plot(x, y, 'white.', 'MarkerSize', 20); hold on;
plot(x_fit, y_linear, 'r-', 'LineWidth', 2);
plot(x_fit, y_quad, 'b--', 'LineWidth', 2);
plot(x_fit, y_cubic, 'g-', 'LineWidth', 2);

grid on
xlabel('Year');
ylabel('Efficiency (%)');
title('Perbandingan regression lines');
